---
title: garrytan/gbrain
created: 2026-04-13
updated: 2026-04-13
type: entity
stars: 6911
github: garrytan/gbrain
tags: [tool, ai-ml]
sources: [raw/stars/2026-04-13.md]
---

# garrytan/gbrain

Opinionated agent configuration. From YC CEO Garry Tan. Reference an opinionated agent setup from a top operator.




## What It Does
Garry Tan's opinionated OpenClaw/Hermes Agent brain configuration. Curated agent setup from Y Combinator's CEO.

## Key Features
- Opinionated agent configuration
- From YC CEO Garry Tan
- Production-tested setup

## Tech Stack
- Language: TypeScript
- License: MIT

## Why I Starred It
6,911 stars.

## Related
- [[anthropics-claude-code]]
- [[obra-superpowers]]

## Use Cases
- Reference an opinionated agent setup from a top operator
- Use as starting point for agent configuration
